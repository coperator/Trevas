...
public class AnalyticCountTest {
  ...
  public void testAnCountRangeWithCalcClause() throws ScriptException {
    ...
    * */
    ...
    engine.eval(
        "res := ds1 [ calc count_Me_1:= count ( Me_1 over ( partition by Id_1,Id_2 order by Year range between 1 preceding and 1 following) )];");
    assertThat(engine.getContext().getAttribute("res")).isInstanceOf(Dataset.class);
  ...
  public void testAnCountWithPartitionClause() throws ScriptException {
    ...
    * */
    ...
    // Transform to res := ds1[ calc count_Me_1 := count ( Me_1 over ( partition by Id_1))];"
  ...
  public void testAnCountWithPartitionOrderByDPClause() throws ScriptException {
    ...
    // Analytical function count test case 3 : count on window with partition, orderBy and data
    // points
    /* Input dataset
    *   +----+----+----+----+----+
        |Id_1|Id_2|Year|Me_1|Me_2|
  ...
  @Test
  public void testAnCountWithPartitionOrderByRange() throws ScriptException {
    // Analytical function count test case 4 : count on window with partition, orderBy and range
    // Because range build window based on the ordered column value, and Id_2 is string, so we
    // change
    ...
    * */
    ...
    engine.eval(
        "res := count ( ds1 over ( partition by Id_1 order by Year range between 1 preceding and 1 following) );");
    assertThat(engine.getContext().getAttribute("res")).isInstanceOf(Dataset.class);
