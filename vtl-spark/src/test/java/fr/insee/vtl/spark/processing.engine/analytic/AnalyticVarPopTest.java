...
public class AnalyticVarPopTest {
  ...
  public void testAnVarPopWithCalcClause() throws ScriptException {
    ...
    * */
    ...
    engine.eval(
        "res := ds1 [ calc var_pop_Me_1:= var_pop ( Me_1 over ( partition by Id_1,Id_2 order by Year) )];");
  ...
  public void testAnVarPopWithPartitionOrderByDPClause() throws ScriptException {
    ...
    // Analytical function count test case 3 : var_pop on window with partition, orderBy and data
    // points
    /* Input dataset
    *   +----+----+----+----+----+
        |Id_1|Id_2|Year|Me_1|Me_2|
  ...
  public void testAnVarPopWithPartitionOrderByRangeClause() throws ScriptException {
    ...
    * */
    ...
    engine.eval(
        "res := var_pop ( ds1 over ( partition by Id_1 order by Year range between 1 preceding and 1 following) );");
    assertThat(engine.getContext().getAttribute("res")).isInstanceOf(Dataset.class);
